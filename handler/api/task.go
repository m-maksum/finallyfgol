...
func (t *taskAPI) UpdateTask(c *gin.Context) {
	var updateTasks model.Task
	if err := c.ShouldBindJSON(&updateTasks); err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: "Invalid request body"})
		return
	}

	idTasks, err := strconv.Atoi(c.Param("id"))
	if err != nil || idTasks != updateTasks.ID {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: "Invalid task ID"})
		return
	}

	err = t.taskService.Update(idTasks, &updateTasks)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
...
func (t *taskAPI) DeleteTask(c *gin.Context) {
	...
	err = t.taskService.Delete(idTasks)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
...
func (t *taskAPI) GetTaskByID(c *gin.Context) {
	...
	task, err := t.taskService.GetByID(idTasks)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
...
func (t *taskAPI) GetTaskList(c *gin.Context) {
	listTasks, err := t.taskService.GetList()
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}
...
func (t *taskAPI) GetTaskListByCategory(c *gin.Context) {
	...
	getTasks, err := t.taskService.GetTaskCategory(idCat)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}

	c.JSON(http.StatusOK, getTasks)
}
